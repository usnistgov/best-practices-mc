...
\begin{equation}
  \alpha_{o\rightarrow n}=\alpha_{n\rightarrow o} = \frac{\diff\theta}{3 N_m \delta\theta_{max}}.
...
\begin{tabular}{|c|c|}
 ...
 \hline
 \makecell{Choose rotation axis} & \makecell{$1/3$} \\
 \hline
...
caption={MC simulation of a rigid body of three sites in an external field using Python 3, with a rotation displacement trial about one of the three Cartesian axes. The average of $\cos\theta$ is compared against the Langevin function.}
...
\lstinputlisting[
language=Python,
label={sim:orientation_uniform},
caption={Validation of the rotation displacement trial of Code Block~\ref{sim:orientation_trials} with no field, where every trial is accepted. Angle distributions were computed over $300$ equally-spaced bins with $20$ independent simulations to compare against theoretical expectations with approximately 68 \% of the distribution within one standard deviation.}
]{../codes/orientation_uniform.py}
\end{figure}
...
\caption{
  The (blue) azimuthal and (red) polar probability distributions of the first vector and (orange) azimuthal angle of the second vector in the reference frame of the first vector from MC simulations described in Code Block~\ref{sim:orientation_uniform} with error bars from the standard deviation of the mean of $20$ independent simulations, and analytical expectations shown by the black lines.
